...
def migrate_teams(args):
	...
	dst_teams = [t['name'] for t in dst.client.get('/orgs/{0}/teams'.format(dst.organisation)).json]
		...
		if src_team['name'] in dst_teams:
			...
			for src_member in src.client.get('/teams/{0}/members'.format(src_team['id'])).json:
				if src_member['login'] in members:
					try:
						dst.client.put('/teams/{0}/members/{1}'.format(dst_team['id'], members[src_member['login']]), data=' ')
						log.debug("User '{0}' added to '{1}'".format(src_member['login'], dst_team['name']))
			...
			for src_repo in src.client.get('/teams/{0}/repos'.format(src_team['id'])).json:
				try:
					dst.client.put('/teams/{0}/repos/{1}'.format(dst_team['id'], dst.full_name(src_repo['name'])), data=' ')
					log.debug("Project '{0}' to '{1}'".format(src_repo['name'], dst_team['name']))
				except APIError as e:
					if e.response.status_code == 404:
